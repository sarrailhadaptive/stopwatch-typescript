...
test('Check correct colors and classnames for components', async ({ page }) => {
  ...
  await page.click('button.stopwatch-button-start')
  const buttonLap = page.getByRole('button', { name: 'Lap', exact: true})
  await expect(buttonLap).toHaveClass('stopwatch-button-lap')
  await page.click('button.stopwatch-button-lap')
  await page.click('button.stopwatch-button-lap')
  await page.click('button.stopwatch-button-lap')
  await page.screenshot({ path: './src/screenshots/check-classes/lap.png' })
  ...
  await page.click('button.stopwatch-button-stop')
  const buttonReset = page.getByRole('button', { name: 'Reset', exact: true})
  await expect(buttonReset).toHaveClass('stopwatch-button-reset')
  await page.screenshot({ path: './src/screenshots/check-classes/pre-reset.png' })
  await page.click('button.stopwatch-button-reset')
  await page.screenshot({ path: './src/screenshots/check-classes/post-reset.png' })
})